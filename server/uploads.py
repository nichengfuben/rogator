...
"""简化版 QwenClient 的文件/图片上传能力（STS 凭证获取 + OSS 上传）。

对齐 .repo/Provider-Qwen-Adapter 的 core/media/upload.py：
STS 临时凭证获取、OSS PUT 上传、按文件类型限制大小、base64 图片解析与
消息内联图片提取、生成图回存下载。
"""
...
from server.session_store import QwenSession
