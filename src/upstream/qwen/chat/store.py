...
SESSIONS_FILE: str = "persist/qwen/sessions.json"
CLEANUP_INTERVAL: float = 60.0
_save_lock = threading.Lock()


def _atomic_write_text(path: Path, content: str) -> None:
    """原子写入文本；Windows 上 replace 被拒时回退为原地写入。"""
...
def save_sessions(
    ...
    *,
    ...
    with _save_lock:
        try:
            _atomic_write_text(Path(SESSIONS_FILE), payload)
        except OSError as e:
            logger.debug("Failed to save sessions: %s", e)
    return removed
