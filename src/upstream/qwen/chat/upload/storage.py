...
def _make_path(save_dir: str, prefix: str, ext: str) -> Path:
    ...
    return (
        Path(save_dir)
        / f"{prefix}_{int(time.time() * 1000)}_{uuid.uuid4().hex[:8]}{ext}"
    )
...
def save_video_file(
    video_data: bytes, save_dir: str = GENERATED_VIDEO_DIR
) -> Optional[str]:
...
def build_oss_authorization(
    ...
    if oss_headers:
        canonicalized = (
            "\n".join(f"{key}:{value}" for key, value in sorted(oss_headers.items()))
            + "\n"
        )
