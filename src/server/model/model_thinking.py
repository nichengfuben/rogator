...
from server.model.model_registry import ModelNotConfiguredError, uses_entml_thinking
...
def resolve_qwen_thinking(
    ...
    *internal_model* 必须为注册表解析后的上游 ID。
    ...
    if uses_entml_thinking(internal_model):
...
def model_supports_thinking(internal_model: str) -> bool:
    if internal_model in _ALWAYS_QWEN_THINKING_MODELS:
        return True
    try:
        return uses_entml_thinking(internal_model)
